...
- [ ] **Vault: responsive / mobile strategy** -- the interactive scene is fundamentally a desktop experience. Decision made: below ~900px, show an alternate layout (vault bg as decorative hero banner, scrollable vertical project card list below it, cats tappable in hero). Desktop keeps the full interactive scene. This is a deliberate design decision, not a fallback -- document it in the blog post when built. This is a dedicated session's worth of work; plan it before starting. Key things to resolve: (1) image crop at portrait ratio, (2) card list layout on mobile, (3) whether the cinematic transition plays on mobile or skips straight to the vault, (4) hotspot editor is desktop-only and doesn't need changing.
